import { startDGLabServer, stopDGLabServer, sendCommandToApps, generateControlId, serverEvents, getConnectionStatus } from './server';
import { generateQrCodeImage } from './qrcode';
import { pulseCmd, MAX_PULSE_PER_SEND } from './protocol';
import { EmotionEngine } from './emotion';
import { testWave, punishWave, teaseWave } from './waveforms';
import { autoLoadFromDir, loadPulsesFile, parsePulsesContent, getLibrary, findPreset, addPreset, removePreset, exportLibrary, PulsePreset } from './pulselib';
...
// ─── 波形连续发送管理 ───
// 对于超过 7s 的波形，分段发送，间隔略小于段时长以保持连续
interface PulseQueue {
  channel: 'A' | 'B';
  segments: string[][];
  currentIndex: number;
  timer: NodeJS.Timeout | null;
}
const activePulseQueues: Map<string, PulseQueue> = new Map(); // key = channel

function stopPulseQueue(channel: 'A' | 'B') {
 ...
 */
function sendPulseWithContinuity(channel: 'A' | 'B', hexArray: string[]) {
  stopPulseQueue(channel); // 清除之前的队列

  if (hexArray.length <= MAX_PULSE_PER_SEND) {
    // 短波形，直接发
    sendCommandToApps(pulseCmd(channel, hexArray));
    return;
  }

  // 分段
  const segments: string[][] = [];
  for (let i = 0; i < hexArray.length; i += MAX_PULSE_PER_SEND) {
    segments.push(hexArray.slice(i, i + MAX_PULSE_PER_SEND));
  }

  const firstSegment = segments[0]!;

  // 立即发第一段
  sendCommandToApps(pulseCmd(channel, firstSegment));

  if (segments.length <= 1) return;

  const queue: PulseQueue = {
    channel,
    segments,
    currentIndex: 1,
    timer: null,
  ...
  queue.timer = setInterval(() => {
    ...
    sendCommandToApps(pulseCmd(channel, queue.segments[queue.currentIndex]!));
    queue.currentIndex++;
 ...
 * 应用强度限制后发送强度指令
 */
function applyStrength(channel: 'A' | 'B', delta: number): number {
  ...
  if (delta === 0) {
    if (channel === 'B') currentIntensityB = 0;
    else currentIntensityA = 0;
    sendCommandToApps(`strength-${ch}+2+0`);
    return 0;
  }

  let target = current + delta;
  if (target > effectiveLimit) target = effectiveLimit;
  if (target < 0) target = 0;
  const actualDelta = target - current;

  if (channel === 'B') currentIntensityB = target;
  else currentIntensityA = target;

  if (actualDelta === 0) return 0;

  if (actualDelta > 0) {
    sendCommandToApps(`strength-${ch}+1+${actualDelta}`);
  } else {
    sendCommandToApps(`strength-${ch}+0+${Math.abs(actualDelta)}`);
  }
  return actualDelta;
}

function checkSafetyLimits() {
  if (!sessionStartTime || !isEmotionModeOn) return;
  const now = Date.now();
  ...
  if (!isResting && elapsed >= MAX_ACTIVE_MS) {
    ...
    applyStrength('A', 0);
    applyStrength('B', 0);
    sendCommandToApps('clear-1');
    sendCommandToApps('clear-2');
    stopPulseQueue('A');
    stopPulseQueue('B');
...
export default function registerPlugin(api: any) {
  ...
  api.registerService({
    id: "dg-lab-ws",
    start: () => {
      startDGLabServer(port);
      timerInterval = setInterval(checkSafetyLimits, 10000);

      // 监听 APP 上报的设备真实强度
      serverEvents.on('strength-feedback', (data: { strengthA: number; strengthB: number; limitA: number; limitB: number }) => {
      ...
      const count = autoLoadFromDir(pulseDir);
  ...
  api.registerCommand({
    ...
    handler: async (_ctx: any) => {
      ...
      const imagePath = await generateQrCodeImage(wsUrl, controlId, mediaDir);
  ...
  api.registerCommand({
    ...
    handler: async (ctx: any) => {
      ...
      limitIntensity = val;
      if (currentIntensityA > limitIntensity) applyStrength('A', limitIntensity - currentIntensityA);
      if (currentIntensityB > limitIntensity) applyStrength('B', limitIntensity - currentIntensityB);
  ...
  api.registerCommand({
    ...
    handler: async (ctx: any) => {
      ...
      const val = parseInt(arg);
      if (isNaN(val)) return { text: '参数格式错误喵！用法: /dg_test +5 或 /dg_test -3' };
  ...
  api.registerCommand({
    ...
    handler: async (_ctx: any) => {
      ...
      if (queueA || queueB) {
        lines.push(`── 波形队列 ──`);
        if (queueA) lines.push(`A: 段 ${queueA.currentIndex}/${queueA.segments.length}`);
        if (queueB) lines.push(`B: 段 ${queueB.currentIndex}/${queueB.segments.length}`);
      }
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      ...
      // Generate waveform
      let wave: string[];
      const preset = findPreset(waveform);
      if (preset) {
        wave = preset.pulseData;
      } else {
        switch (waveform) {
          case 'tease': wave = teaseWave(duration); break;
          case 'test': wave = testWave(); break;
          default: wave = punishWave(duration); break;
        }
      }
  ...
  api.registerCommand({
    ...
    handler: async (ctx: any) => {
      ...
      if (sub === 'load') {
        ...
        if (!filePath) return { text: '用法: /dg_pulse load <文件路径>' };
        ...
        try {
          const presets = loadPulsesFile(resolvedPath);
      ...
      if (sub === 'delete' || sub === 'del' || sub === 'rm') {
        const id = args[1];
        if (!id) return { text: '用法: /dg_pulse delete <id>' };
        if (removePreset(id)) {
      ...
      if (sub === 'play') {
        ...
        if (!name) return { text: '用法: /dg_pulse play <名称或ID>' };
        const preset = findPreset(name);
        if (!preset) return { text: `未找到波形: ${name}` };
  ...
  api.registerTool({
    ...
    async execute() {
      ...
      const custom = getLibrary().map(p => `${p.name} [${p.id}] (${p.pulseData.length} frames, ${(p.pulseData.length * 0.1).toFixed(1)}s)`);
  ...
  api.registerHook("message:sent", async (event: any) => {
    ...
    const hexArray = EmotionEngine.generateWaveformForEmotion(score);

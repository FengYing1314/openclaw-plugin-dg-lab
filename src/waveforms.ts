...
import { compressFrequency } from './protocol';
 ...
 */
function waveHex(freqMs: number, level: number): string {
  const f = compressFrequency(freqMs);
 ...
 */
export function punishWave(durationMs: number = 1000): string[] {
 ...
 * 不限制长度，由 sendPulseWithContinuity() 自动分段发送
 */
export function teaseWave(durationMs: number = 1000): string[] {
  const count = Math.max(1, Math.floor(durationMs / 100));
  const result: string[] = [];
  for (let i = 0; i < count; i++) {
 ...
 * 测试波形: 中频短促 (0.5秒)
 */
export function testWave(): string[] {
  return [

...
export function strengthCmd(channel: '1' | '2', mode: '0' | '1' | '2', value: number): string {
  return `strength-${channel}+${mode}+${value}`;
}

export function clearQueueCmd(channel: '1' | '2'): string {
 ...
 */
export function pulseCmd(channel: 'A' | 'B', waveformHexArray: string[]): string {
  const capped = waveformHexArray.slice(0, MAX_PULSE_PER_SEND);
  ...
  if (cmd.length > MAX_MESSAGE_LENGTH) {
    console.warn(`[DG-Lab Protocol] pulse command length ${cmd.length} exceeds ${MAX_MESSAGE_LENGTH}, truncating`);
    // 逐步减少直到合法
    let arr = capped;
    while (arr.length > 1) {
      arr = arr.slice(0, arr.length - 1);
      const shortened = `pulse-${channel}:${JSON.stringify(arr)}`;
      if (shortened.length <= MAX_MESSAGE_LENGTH) return shortened;
    }
  }
  return cmd;
}
 ...
 * 根据官方 V3 文档的转化公式
 */
export function compressFrequency(input: number): number {
 ...
 */
export function decompressFrequency(encoded: number): number {
  if (encoded <= 100) {
    return encoded;
  } else if (encoded <= 200) {
...
// 4. Bluetooth Layer Utilities

export interface WavePoint {
  freq: number;   // Frequency (10-1000, will be compressed)
  level: number;  // Intensity (0-100)
}

export function generateWaveformHex(points: [WavePoint, WavePoint, WavePoint, WavePoint]): string {
  let hex = '';
  // 前4字节: 频率 (压缩后)
